...
class AddressRepositoryImpl(AddressRepository):
    ...
    def get_by_user_id(self, user_id: UUID) -> list[AddressEntity]:
        address_model = (
            self.db.query(AddressModel).filter(AddressModel.user_id == user_id).all()
        )
    ...
    def get_by_id(self, id: UUID) -> AddressEntity | None:
        address_model = (
            self.db.query(AddressModel).filter(AddressModel.id == id).first()
        )
    ...
    def delete(self, user_id: UUID) -> None:
        profile = (
            self.db.query(AddressModel).filter(AddressModel.user_id == user_id).first()
        )
